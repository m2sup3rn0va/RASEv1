...
Function Initialize-RootAVD
{
    ...
    $avdArch = (Get-Content "$avdDataPath\$avd.avd\config.ini" | findstr "hw.cpu.arch").split('=')[1]
    Write-Host
    ...
    Get-ChildItem .\auto_install\ | findstr /RI ".*.apk$" | findstr /I "supersu" 2>&1 | Out-Null
    if ($LASTEXITCODE)
    {
        Write-Host
    ...
    Get-ChildItem .\auto_install\ | findstr /RI ".*.zip$" | findstr /I "supersu" 2>&1 | Out-Null
    if ($LASTEXITCODE)
    {
        Write-Host
    ...
    Get-ChildItem .\auto_install\ | findstr /RI ".*.apk$" | findstr /I "rootchecker" 2>&1 | Out-Null
    if ($LASTEXITCODE)
    {
        Write-Host
    ...
    Expand-Archive -Path .\auto_install\supersu.zip -DestinationPath .\auto_install\supersu\
    if ($LASTEXITCODE)
    {
        Write-Host
        Write-Host "[+] Unable to UNZIP SuperSu. Please unzip it manually"
        Write-Host
        Write-Host "[+]================================================================================================================================"
        break
    }
    ...
    Write-Host
    Write-Host "[+] Remount the File-System"
    adb.exe -s $emulatorName remount 2>&1 | Out-Null
    ...
    Write-Host
    Read-Host -Prompt "[+] Hit Enter if we are good"
    ...
    adb.exe -s $emulatorName shell "nohup $avdSULocation/su 0 su --daemon &"
    Start-sleep -s 2
